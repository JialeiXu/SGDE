...
string root_path = "../../LoFTR/save_matching_points/nuScenes/6Camera_7Frame/USAC_ACCURATE/";
    ...
    public:
        ...
        bool LoadFile() {
            ...
            for(auto iter = Cam1ToCam_i.begin();iter!=Cam1ToCam_i.end();iter++){
                for(int k=0;k<6;k++){
                    parameters_[map_cam_num[iter->first]*6+k] = iter->second[k];
...
struct SnavelyReprojectionError_Cam_01 {
    ...
    template <typename T>
    bool operator()(const T* const point, const T* const cam01_pose, T* residuals) const {
        T p[3];
    ...
    return (new ceres::AutoDiffCostFunction<SnavelyReprojectionError_Cam_01, 2, 3, 6>(
...
struct SnavelyReprojectionError_Cam_else {
    ...
    bool operator()(const T* const point, const T* const cam01_pose, const T* const poseCam1ToCam_i, T* residuals) const {
        ...
        // the camera model that Noah Snavely's Bundler assumes, whereby
        // the camera coordinate system has a negative z axis.
        ceres::AngleAxisRotatePoint(poseCam1ToCam_i, p_tmp, p);
    ...
    return (new ceres::AutoDiffCostFunction<SnavelyReprojectionError_Cam_else, 2, 3, 6, 6>( //2,3,6,6 最后一个数字，6表示优化poseCam1ToCam_i的R+T，3表示只优化R
